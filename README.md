...
│ ├── app.py \# Flask app với /metrics, /healthz, ERROR_RATE inject
...
│ ├── web.yaml \# ConfigMap + Deployment nginx + Service (wave 0/1/2)

│ └── \...

├── k8s-api/

│ ├── api.yaml \# Rollout (canary) + Service

│ ├── servicemonitor.yaml \# Prometheus scrape /metrics mỗi 15s

│ ├── analysis-template.yaml \# AnalysisTemplate --- query Prometheus tự
chấm

│ └── slo-rule.yaml \# PrometheusRule --- SLO burn rate alert

├── argocd/

│ ├── root.yaml \# App-of-apps --- apply 1 lần, quản tất cả
...
└── validate.yml \# CI: kubeconform validate YAML trước khi merge
...
Dùng \*\*multi-window burn rate\*\* thay vì threshold đơn giản --- bắt
được cả incident gấp (fast) lẫn rò rỉ âm ỉ (slow).

\| Loại \| Cửa sổ \| Ngưỡng burn rate \| Ý nghĩa \|

\|\-\--\|\-\--\|\-\--\|\-\--\|

\| Fast (critical) \| 1h \| \> 14.4× \| Error budget cháy hết trong \~2
giờ \|
...
Ngưỡng 14.4× trên cửa sổ 1h là ngưỡng chuẩn từ Google SRE Book cho
\"fast burn\" --- đủ nhạy để alert sớm nhưng không quá nhiều false
positive.
...
\# Mong đợi: AnalysisRun Degraded → Rollout Aborting → về bản cũ trong
\< 3 phút
...
![](./media/image3.png){width="5.769444444444445in"
height="1.9847222222222223in"}

![](./media/image4.png){width="5.769444444444445in"
height="2.622916666666667in"}

![](./media/image5.png){width="5.769444444444445in" height="2.6in"}

![](./media/image6.png){width="5.761805555555555in"
height="3.845833333333333in"}

![](./media/image7.png){width="5.769444444444445in"
